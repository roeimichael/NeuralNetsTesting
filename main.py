...
def create_model_configs(models, skip_models, cost_sensitive_loss_functions, optimizers,
    ...
    """
    Creates a list of ModelConfig objects for every combination of model, parameters, loss function, and optimizer.
    """
    return [
        ModelConfig(model_info, params, loss_function_info, optimizer_info, optimizer_p,
                    float(loss_function_info["name"].split('_')[1]), train_dl, test_dl, n_epochs, next_day_data_path,
                    device, create_save_directory(model_info["name"]), model_info["class"], optimizer_info["class"])
        for model_info in models if model_info["name"] not in skip_models
        for params in ParameterGrid(model_info["params"])
        for loss_function_info in cost_sensitive_loss_functions
        for optimizer_info in optimizers
        for optimizer_p in ParameterGrid(optimizer_info["params"])
    ]


def get_data_paths(starting_date):
    """
    Retrieves the paths to train, test, and next day data based on a starting date.
    """
...
def main(n_epochs=100):
    ...
    starting_date = 466
    data_paths = get_data_paths(starting_date)
    next_day_data_path = data_paths["next_day_data_path"]
    data_paths = {k: v for k, v in data_paths.items() if k != "next_day_data_path"}
    train_dl, test_dl = prepare_data(**data_paths, starting_date=starting_date)
    skip_models = []
